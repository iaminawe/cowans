...
class ImageGenerationRequest:
    ...
    model: str = "gpt-image-1"  # "gpt-image-1" or "dall-e-3"
    
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary for API request."""
        data = {
            "model": self.model,  # Use specified model (gpt-image-1 or dall-e-3)
            "prompt": self.prompt,
            "size": self.size,
            "n": self.n
        }
        # Don't include quality parameter for compatibility with older OpenAI versions
        return data
    
    def get_cache_key(self) -> str:
        """Generate cache key for the request."""
        key_data = f"{self.prompt}_{self.size}_{self.quality}_{self.style}_{self.model}"
...
class OpenAIImageClient:
    ...
    async def generate_image(self, request: ImageGenerationRequest) -> ImageGenerationResult:
        ...
        for attempt in range(Config.OPENAI_MAX_RETRIES):
            try:
                ...
                # Handle base64 encoded image (gpt-image-1 format)
                if hasattr(image_data, 'b64_json') and image_data.b64_json:
                    local_path = await self._save_base64_image(
                        image_data.b64_json, request.category, cache_key
                    )
                # Handle URL format (DALL-E 3 format)
                elif hasattr(image_data, 'url') and image_data.url:
                    local_path = await self._download_and_save_image(
                        image_data.url, request.category, cache_key
                    )
                else:
                ...
                result = ImageGenerationResult(
                    success=True,
                    image_url=getattr(image_data, 'url', None),  # URL for DALL-E 3, None for gpt-image-1
                    ...
                    metadata={
                        ...
                        'attempt': attempt + 1,
                        'format': 'base64' if hasattr(image_data, 'b64_json') else 'url'
                    },
                    generation_time=generation_time
                )
    ...
    async def _save_base64_image(self, base64_data: str, category: str, request_id: str) -> str:
        """Save base64 encoded image locally."""
        try:
            # Decode base64 data
            ...
            logger.debug(f"Saved base64 image: {local_path}")
